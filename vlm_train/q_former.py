...
def create_attention_mask(
    ...
    img_cross = multimodal_cross_fn(
        B, T, I,
        device=device,
        dtype=torch.bool
    )
    text_cross = multimodal_cross_fn(
        B, I, T,
        device=device,
        dtype=torch.bool
    )
    ...
    def forward(self, visual_feats, 
        ...
        for i, layer in enumerate(self.encoder_layers):
            ...
            if str(i) in self.cross_blocks:
                ...
                queries = self.cross_blocks[str(i)](
                    queries, visual_feats)
...
if __name__ == "__main__":
    ...
    qformer.eval()
    with torch.no_grad():
        out_queries, out_text = qformer(image_feats, 
                                input_ids, 
                                text_attention_mask,
    ...
    loaded_qformer.eval()
    with torch.no_grad():
        out_queries_loaded, out_text_loaded = loaded_qformer(image_feats, 
                                input_ids, 
                                text_attention_mask,
                                attention_mode="multi_modal_causal"
                                    )
